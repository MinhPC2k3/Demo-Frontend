...
async function main(){
  ...
  const BeforeTransfer = await simpleTransaction.CheckAccount(account0.address)
  ...
  await simpleTransaction.deposit({ value: ethers.utils.parseEther(etherAccount1Deposit1.toString()) });
  ...
  const currentValue2 = await simpleTransaction.Retrieve(account0.address)
  ...
  // console.log(`${Money}`)
  // saveFrontendFiles(simpleTransaction,SimpleTransaction)
}
